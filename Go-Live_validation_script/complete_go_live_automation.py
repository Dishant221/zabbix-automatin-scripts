...
def update_hostgroup_to_golive(zapi, host, old_hostgroup_name):
    ...
    try:
        ...
        golive_group_name = f"GO-LIVE/{portfolio}/{product}/{hub_id}/E2Customer"

    except Exception:
        print(f"❌ Invalid HG format: {old_hostgroup_name}")
        return False
    ...
    if existing:
        ...
        print(f"✅ Created new HG: {golive_group_name}")
    ...
    for g in current_groups:
        ...
        if name.startswith("UAT/") or name.startswith("CI-PROD-PRODUCTION/"):
            print(f"🗑️ Removing group {name} from {hostname}")
            continue
    ...
    try:
        ...
        print(f"🎉 {hostname} moved to GO-LIVE group")
        return True

    except Exception as e:
        print(f"❌ Failed updating host {hostname}: {e}")
        return False

def perform_go_live(zapi, hostgroup_name, dry_run=False):
    """
    ...
    for hostname, p in all_active_problems:
        ...
        try:
            ...
            print(f"⚠️ Failed to close {event_id}: {e}")

    # -------------------------------
    # 🔁 Rename Hostgroup
    # -------------------------------
    print("\n⚙️ Renaming hostgroup...\n")

    hg = zapi.hostgroup.get(filter={"name": [hostgroup_name]}, output=["groupid", "name"])

    if not hg:
        print("❌ Hostgroup not found")
        return False
    ...
    try:
        ...
        print("❌ Invalid hostgroup naming format")
        return False
    ...
    try:
        zapi.hostgroup.update(groupid=group_id, name=new_name)
...
def main():
    ...
    perform_go_live(zapi, hostgroup_name, dry_run=False)
