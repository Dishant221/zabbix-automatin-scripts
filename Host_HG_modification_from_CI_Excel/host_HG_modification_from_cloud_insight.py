...
def seperate_common_VMs(CloudInsights_monitored_VM_list, Zabbix_monitored_VM_list):
    ...
    ci_vm_names = {vm['host_name'] for vm in CloudInsights_monitored_VM_list}
    zb_vm_names = {vm['name'].split('.', maxsplit=1)[0] for vm in Zabbix_monitored_VM_list}

    for vm in CloudInsights_monitored_VM_list:
        if vm['host_name'] in [zb_vm_name for zb_vm_name in zb_vm_names]:
            common_VMs.append(vm)
        else:
            ci_only_VMs.append(vm)

    for vm in Zabbix_monitored_VM_list:
        if vm['name'].split('.', maxsplit=1)[0] in ci_vm_names:
...
def change_hostgroup_of_host(zapi,ci_common_VMs,zb_only_VMs):
    ...
    for vms in ci_common_VMs:
        updated_hostgroup_name = 'UAT/CI-PROD-PRODUCTION/HUB' + '/' + vms['hub_type'] + '/' + vms['hub_identifier'] + '/' + vms['e2customer_hub']
        try:
            ...
            print("Error updating host '{}': {}".format(vms['host_name'], e))
            logging.error("Error updating host '%s': %s", vms['host_name'], e)

        for zb_vm in zb_only_VMs:
            zb_vm_name_only = zb_vm['name'].split('.', maxsplit=1)[0]
            #adding new host group to the host
            if vms['host_name'] == zb_vm_name_only:
                ...
                try:
                    ...
                    print("Updated host '{}' to new host group '{}'".format(vms['host_name'], updated_hostgroup_name))
                    logging.info("Updated host '%s' to new host group '%s'", vms['host_name'], updated_hostgroup_name)
                except ZabbixAPIException as e:
                    print("Error updating host '{}': {}".format(vms['host_name'], e))
...
if __name__ == "__main__":
    ...
    print("--------------------Script started at {}---".format(date))
    logging.info("--------------------Script started at {}---".format(date))
