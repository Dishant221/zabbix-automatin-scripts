...
def get_licenses(api_url, token, sample_size=500):
    endpoint = "/api/sam/v1/licenses"
...
def main():
    ...
    total_items, sample_records, url = get_licenses(api_url, token)

    # Save results to CSV
    csv_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'csv_files')
    os.makedirs(csv_dir, exist_ok=True)
    csv_path = os.path.join(csv_dir, 'licenses.csv')
