...
#DEV_ZABBIX_URL = 'https://monitor.e2open.com/zabbix/api_jsonrpc.php'
...
logging.basicConfig(filename='golive_to_decommissioned.log',
                    filemode='a',
...
def update_host_description(hostid, new_entry):
    ...
    # Step 2: Append new entry with underscore
    if current_desc:
        updated_desc = f"{current_desc}_{new_entry}"
    else:
    ...
    logging.info(f"✅ Host description updated to: {updated_desc}")
...
def process_host_groups(hostname):
    ...
    if not original_groups:
        print(f"❌ Host '{hostname}' has no groups assigned. Cannot proceed.")
    ...
    # If no GO-LIVE group found, add generic 'DECOMMISSIONED' group
    if not go_live_found:
        generic_group = get_group_by_name("DECOMMISSIONED")
        if not generic_group:
            generic_group = create_group("DECOMMISSIONED")
            print(f"✅ Created group: DECOMMISSIONED")
            logging.info(f"✅ Created group: DECOMMISSIONED")

        updated_groupids.append(generic_group['groupid'])

    update_host_groups(hostid, updated_groupids)
    print(f"✅ Host '{hostname}' group(s) updated.")
    logging.info(f"✅ Host '{hostname}' group(s) updated.")
