...
def get_host_from_hostgroup(zapi, hostgroup_name):
    ...
    try:
        ...
        hosts = zapi.host.get(groupids=hostgroup_id, output=["hostid", "name","status"], selectGroups="extend", selectHostGroups="extend")
        return hosts
...
def add_tag_to_host(zapi, hostid, new_tag_with_no_values):
    ...
    if not host:
        msg = f" No host found for hostid {hostid}"
        print(msg)
        logging.info(msg)
    ...
    if new_tags_added == 0 and tags_updated == 0:
        msg = f" No tag changes needed, skipping.."
        print(msg)
        logging.info(msg)
    ...
    try:
        ...
        msg = f" {hostname}: Added {new_tags_added} new tag(s)"
        print(msg)
        logging.info(msg)
...
def add_automation_hg(zapi, host_id, hostname):
    #automation_hg_name = "GO-LIVE-SCRIPT-VM-ENABLED"
    #automation_hostgroup_id = "16181" #groupid for GO-LIVE-SCRIPT-VM-ENABLED
    today = datetime.now()
    # isocalendar() returns (ISO year, ISO week number, ISO weekday)
    iso_calendar = today.isocalendar()
    week_number = iso_calendar[1]
    month_number = today.month
    automation_hg_name = f"GO_LIVE_SCRIPT_VM_ENABLED_W{week_number}_{month_number}"
    ...
    if not host_data:
        ...
        logging.error(f"Host not found for automation HG: {hostname}")
        return

    existing_groups = host_data[0].get("groups", [])

    for group in existing_groups:
        if group["name"] == automation_hg_name:
            print(f"Automation host group {automation_hg_name} already exists for {hostname}, skipping addition.")
            logging.info(f"Automation host group {automation_hg_name} already exists for {hostname}, skipping addition.")
            return

    # Look up or create the automation host group to get its groupid
    existing_hg = zapi.hostgroup.get(filter={"name": automation_hg_name})
    if existing_hg:
        automation_groupid = existing_hg[0]["groupid"]
    else:
        new_hg = zapi.hostgroup.create(name=automation_hg_name)
        automation_groupid = new_hg["groupids"][0]
        logging.info(f"Created automation host group: {automation_hg_name} (ID: {automation_groupid})")

    # Build group list using groupid only
    updated_groups = [{"groupid": g["groupid"]} for g in existing_groups]
    updated_groups.append({"groupid": automation_groupid})
    ...
    try:
        ...
        print(f"Added automation host group '{automation_hg_name}' to {hostname}")
        logging.info(f"Added automation host group '{automation_hg_name}' to {hostname}")
    except ZabbixAPIException as e:
        print(f"Error adding automation host group to {hostname}: {e}")
        logging.error(f"Error adding automation host group to {hostname}: {e}")
...
def update_hostgroup_to_golive(zapi, host, old_hostgroup_name):
    logging.info("__________UAT to GOLIVE_______")
    #logging.info(f"Updating host group for {host['host']} to GO-LIVE format based on old host group {old_hostgroup_name}")
    """
    Remove UAT/CI-PROD groups from host and add GO-LIVE group
    """
    ...
    if not host_data:
        ...
        logging.error(f"Host not found: {hostname}")
        return False

    # Fetch groups independently via hostgroup.get (works across all Zabbix versions)
    current_groups = zapi.hostgroup.get(hostids=host_id, output=["groupid", "name"])
    logging.info(f"Current groups for {hostname}: {[g['name'] for g in current_groups]}")

    # Skip if host already has a GO-LIVE group
    for g in current_groups:
        if g["name"].startswith("GO-LIVE/"):
            msg = f" Skipping {hostname}: already in GO-LIVE group '{g['name']}'"
            print(msg)
            logging.info(msg)
            return False

    #if host is enabled (status=0) proceed, otherwise skip
    if host_data[0].get("status") == "0":
        #print(f"\n Processing host: {hostname} (enabled)")
        logging.info(f"Processing host: {hostname} (enabled)")
        logging.info(f"Groups for {hostname}: {[g['name'] for g in current_groups]}")

        # Extract GO-LIVE name

        parts = old_hostgroup_name.split("/")
        ...
        try:
            ...
            golive_group_name = f"GO-LIVE/{portfolio}/{product}/{hub_id}/{e2_customer}"

        except Exception:
            print(f"\n Invalid HG format: {old_hostgroup_name}")
            logging.error(f"Invalid HG format: {old_hostgroup_name}")
            return False

        # Ensure GO-LIVE group exists

        existing = zapi.hostgroup.get(filter={"name": [golive_group_name]})

        if existing:
            golive_group_id = existing[0]["groupid"]
        else:
            new_group = zapi.hostgroup.create(name=golive_group_name)
            golive_group_id = new_group["groupids"][0]
            print(f"\n CREATED NEW HG: {golive_group_name}")
            logging.info(f"Created new HG: {golive_group_name}")
        # Filter groups
        ...
        for g in current_groups:
            ...
            #  Remove only UAT / CI-PROD groups
            if name.startswith("UAT/") or name.startswith("UAT/CI-PROD-PRODUCTION/"):
                print(f"\n Removing group {name} from {hostname}")
                continue

            #  Keep all other groups
            new_group_ids.append({"groupid": g["groupid"]})

        #  Add GO-LIVE group
        new_group_ids.append({"groupid": golive_group_id})

        # -------------------------------
        # Update host groups
        # -------------------------------
        try:
            hostupdated=zapi.host.update(
                hostid=host_id,
                groups=new_group_ids
            )

            print(f"\n {hostname} moved to GO-LIVE :'{golive_group_name}'")
            logging.info(f"{hostname} moved to GO-LIVE : '{golive_group_name}'")
            #print("+++++++++++++++++++++++++++++++++++++++++++++++")
            print("GO-LIVE process completed.")
            #print("+++++++++++++++++++++++++++++++++++++++++++++++")
            #if hostupdated:
                # removing uat hostgroup after adding go-live hostgroup
            #    delete_empty_uat_hostgroups(zapi, old_hostgroup_name)
            return True

        except Exception as e:
            print(f"\n Failed updating host {hostname}: {e}")
            logging.error(f"Failed updating host {hostname}: {e}")

            return False
        
    else:
        print(f"\n Skipping host: {hostname} (status not enabled)")
        logging.info(f"Skipping host: {hostname} (status not enabled)")
        return False
#--------------------------------------------------------------------------------
def perform_go_live(zapi, hostgroup_name, hosts_collecting_data, dry_run=False):
    #logging.info(f"Starting GO-LIVE for host group: {hostgroup_name} with dry_run={dry_run}")
    ...
    for host in hosts_collecting_data:
        ...
        if host_status == "1":
            print("  Go-live stopped: Host is Disable\n")
            continue  # skip disabled

        print(f"\t #PROCESSING HOST: {hostname} FOR LIVE MONITORING\n")
        logging.info(f"\n================ Processing Host: {hostname} =================\n")

        problems = get_host_problems(zapi, host_id)

        active_problems = problems["active_problems"]
        last7_active = problems["last_7_days_active_problems"]

        total_active = len(active_problems)
        total_last7 = len(last7_active)

        # -------------------------------
        # CONDITION 1 (HARD BLOCK)
        # -------------------------------
        if total_last7 >= 30:
            msg = f"SKIPPED GO-LIVE: {hostname} (hostid: {host_id}) : Last 7 days problems = {total_last7} (>=30)"
            print(f" {msg}")
            logging.warning(msg)
            continue  # skip this host

        # -------------------------------
        #  CONDITION 2 (SOFT WARNING)
        # -------------------------------
        if total_active >= 5:
            msg = f"WARNING: {hostname} (hostid: {host_id}) : Active problems = {total_active} (>=5), still proceeding GO-LIVE"
            print(f" {msg}")
            logging.warning(msg)

        # -------------------------------
        #  CONDITION 3 (SOFT WARNING)
        # -------------------------------
        old_problem_found = False

        for p in active_problems:
            age = now - int(p["clock"])
            if age > six_hours:
                old_problem_found = True
                break

        if old_problem_found:
            msg = f"WARNING: {hostname} (hostid: {host_id}) : Has problem older than 6h, still proceeding GO-LIVE"
            print(f" {msg}")
            logging.warning(msg)
        ...
        if dry_run:
            print(f"DRY RUN: Skipping execution for {hostname}")
            continue

        #print(f"\n Proceeding GO-LIVE for {hostname}")
        ...
        for p in active_problems:
            ...
            try:
                ...
                print(f" Closed problem {event_id}")
                logging.info(f"{hostname}: Closed problem {event_id}")

            except Exception as e:
                print(f" Failed to close {event_id}: {e}")
                logging.error(f"{hostname}: Failed to close {event_id}: {e}")

        # Move to GO-LIVE group
        try:
            hostmade_golive = update_hostgroup_to_golive(zapi, host, hostgroup_name)
            #print(f" {hostname} moved to GO-LIVE")
            #logging.info(f"{hostname}: moved to GO-LIVE group")
            if hostmade_golive:
                add_automation_hg(zapi, host_id, hostname)

        except Exception as e:
            print(f" Failed to update hostgroup Automation: {e}")
            logging.error(f"{hostname}: Failed to update hostgroup Automation: {e}")
...
def main():
    ...
    for uathost_group in child_group:
        time.sleep(3)
        uat_hosts = get_host_from_hostgroup(zapi, uathost_group)
        if uat_hosts:
            print("\n********************************************************************************")
            print(f"Found {len(uat_hosts)} hosts in host group '{uathost_group}'")
            #print("********************************************************************************")
            # ADDING HOSTS TAGS
            print("\n# ADDING HOSTS TAGS\n")
        ...
        hosts_collecting_data, hosts_with_missing_history_not_collecting_data = filter_empty_history_hosts(host_complete_details)


        host_collecting_data_message(hosts_collecting_data)
        host_not_collecting_data_message(hosts_with_missing_history_not_collecting_data)

        hosts = get_hosts_from_group(zapi, uathost_group)
        total_active_problems = 0
        total_last_7_days_active_problems = 0
        total_last_7_days_resolved_problems = 0
        print("# HIGH AND DISASTER TRIGGER: REVIEW ACTIVE PROBLEMS AND HISTORY OF ALERTS GENERATED IN LAST 7 DAYS")
        #{4:<20} |
        print("\n" + "+" + "-"*15 + "+" + "-"*35 + "+" + "-"*18 + "+" + "-"*30 + "+")
        ...
        perform_go_live(zapi, uathost_group,hosts_collecting_data, dry_run=False)
        perform_go_live(zapi, uathost_group,hosts_with_missing_history_not_collecting_data, dry_run=False)
