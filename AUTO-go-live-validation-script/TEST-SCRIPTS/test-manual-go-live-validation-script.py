...
logfilePATH = os.path.join(BaseDirectory,"TEST-GO-LIVE_VALIDATION_LOGS.log")
...
def update_hostgroup_to_golive(zapi, host, old_hostgroup_name):
    ...
    print(f"\t\t Old HostGroup: {old_hostgroup_name}")

    # Get full host details with groups
    ...
    if host_data[0].get("status") == "0":
        ...
        current_groups = host_data[0]["groups"]
        ...
        if existing:
            ...
            print(f"\t\t NewHostGroup Created : {golive_group_name}")
        ...
        for g in current_groups:
            ...
            if name.startswith("UAT/") or name.startswith("UAT/CI-PROD-PRODUCTION/"):
                print(f"\t\t Removing group: {name} from {hostname}\n\n")
                continue
        ...
        try:
            ...
            print(f"\t\t GO-LIVE group : '{golive_group_name}'")
...
def perform_go_live(zapi, hostgroup_name,hosts_collecting_data, dry_run=False):
    logging.info(f"Starting GO-LIVE validation for host group: {hostgroup_name} with dry_run={dry_run}")
    
    Validate and execute GO-LIVE for a hostgroup

    Conditions:
    - Last 7 days (High + Disaster) < 30
    - Active problems < 5
    - Active problem age < 6h (optional)
    - Close all active problems
    - Rename hostgroup to GO-LIVE format
    

    print("\n   Starting GO-LIVE validation...\n")

    #hosts = get_hosts_from_group(zapi, hostgroup_name)
    hosts = hosts_collecting_data
    ...
    for host in hosts:
        ...
        last7_resolved = problems["last_7_days_resolveed_problems"]
        ...
        for p in active_problems:
            all_active_problems.append((hostname, p))

    print(f"Total Active Problems: {total_active}")
    logging.info(f"Total Active Problems: {total_active}")
    print(f"Total Last 7 Days Problems: {total_last7}")
    logging.info(f"Total Last 7 Days Problems: {total_last7}")

    #  Condition 1
    if total_last7 >= 30:
        print("  Go-Live FAILED: Last 7 days problems >= 30")
        logging.info("Go-Live FAILED: Last 7 days problems >= 30")
        return False

    #  Condition 2
    if total_active >= 5:
        print(" Go-Live FAILED: Active problems >= 5")
        logging.info("Go-Live FAILED: Active problems >= 5")
        return False

    #  Condition 3
    for hostname, p in all_active_problems:
        age = now - int(p["clock"])
        if age > six_hours:
            print(f" Go-Live FAILED: Problem older than 6h on {hostname}")
            logging.info(f"Go-Live FAILED: Problem older than 6h on {hostname}")
            return False

    print("\n================ GO-LIVE VALIDATION RESULT ================\n")

    print(f"✔ Condition 1 PASSED: Last 7 days problems ({total_last7}) < 30")
    print(f"✔ Condition 2 PASSED: Active problems ({total_active}) < 5")

    if len(all_active_problems) == 0:
        print("✔ Condition 3 PASSED: No active problems present")
    else:
        print("✔ Condition 3 PASSED: All active problems are < 6 hours old")

    print("\n All GO-LIVE conditions satisfied.")
    print(" Proceeding with GO-LIVE execution...\n")

    print("__________________________________________________________")
    logging.info("All GO-LIVE conditions satisfied. Proceeding execution.")
    ...
    print("\n Closing active problems...\n")
    logging.info("Closing active problems...")
    ...
    for hostname, p in all_active_problems:
        ...
        try:
            zapi.event.acknowledge(
                eventids=[event_id],
                action=5,  # close + message
                message="Auto-closed during GO-LIVE-validation script executions"
            )
            print(f" Closed problem {event_id} ({hostname})")
            logging.info(f"Closed problem {event_id} ({hostname})")

        except Exception as e:
            print(f" Failed to close {event_id}: {e}")
            logging.error(f"Failed to close {event_id}: {e}")
    
    
    for host in hosts: # making host golive by changing hostgroup to golive format
        print("host : {}".format(host))
        update_hostgroup_to_golive(zapi, host, hostgroup_name)

    return True
"""
def evaluate_host_for_golive(zapi, host):
    """
    Returns:
    {
        "can_golive": True/False,
        "reasons": []
        "active_problems": [],
    }
    """
    ...
    total_last7 = len(last7_active) + len(last7_resolved)
    total_active = len(active_problems)
    ...
    # Condition 1
    if total_last7 >= 30:
        reasons.append(f"Last 7 days problems >= 30 ({total_last7})")

    # Condition 2
    if total_active >= 5:
        reasons.append(f"Active problems >= 5 ({total_active})")

    # Condition 3
    for p in active_problems:
        age = now - int(p["clock"])
        if age > six_hours:
            reasons.append("Problem older than 6 hours")
            break

    return {
        "can_golive": len(reasons) == 0,
        "reasons": reasons,
        "active_problems": active_problems
    }

def perform_go_live_per_host(zapi, hostgroup_name, hosts):

    #print("\n========== PER HOST GO-LIVE VALIDATION ==========\n")
    ...
    for host in hosts:
        ...
        if status == "1":
            print(f"⏭️ Skipping {hostname} (Disabled)")
            continue

        result = evaluate_host_for_golive(zapi, host)

        #print("\n--------------------------------------------------")
        print(f"\tHost: {hostname}")

        if result["can_golive"]:
            print("\tAll conditions satisfied. Proceeding GO-LIVE...")
            proceed = "y"
        else:
            print("❌ GO-LIVE BLOCKED due to:")
            for r in result["reasons"]:
                print(f"   - {r}")

            proceed = input("👉 Do you want to FORCE GO-LIVE? (y/n): ").strip().lower()

        if proceed == "y":
            print(f"\t\tExecuting GO-LIVE for {hostname}")

            # Close problems
            for p in result["active_problems"]:
                try:
                    zapi.event.acknowledge(
                        eventids=[p["eventid"]],
                        action=5,
                        message="Force closed during GO-LIVE"
                    )
                    print(f"   Closed problem {p['eventid']}")
                except Exception as e:
                    print(f"   Failed closing problem: {e}")

            # Move to GO-LIVE group
            update_hostgroup_to_golive(zapi, host, hostgroup_name)

        else:
            print(f"⏭️ Skipping GO-LIVE for {hostname}")
...
def main():
    ...
    #getting complete host details hostid, hostname, status, items and item key history    
    host_complete_details = get_all_hosts_details_from_json(zapi, uat_hosts, keys)
    # getting 2  list host which is collecting data not collecting data 
    ...
    #perform_go_live(zapi, hostgroup_name,hosts_collecting_data, dry_run=False)
    print("------------Go-Live Validation For Hosts Collecting Data------------------\n")
    perform_go_live_per_host(zapi, hostgroup_name, hosts_collecting_data)
    print("\n------------Go-Live Validation For Hosts NOT Collecting Data------------------\n")
    perform_go_live_per_host(zapi, hostgroup_name, hosts_with_missing_history)
