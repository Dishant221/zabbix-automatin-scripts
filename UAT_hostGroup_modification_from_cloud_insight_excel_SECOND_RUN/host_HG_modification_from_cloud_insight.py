...
# Excel sheet we expect to read
sheet_name = 'Non Go live powered on VMs'

# Try reading the expected sheet; if missing or any error occurs, list available sheets and exit
try:
    db = xl.readxl(fn=xl_file_path, ws=sheet_name)
except Exception as e:
    print(f"Error reading sheet '{sheet_name}': {e}")
    try:
        db_tmp = xl.readxl(fn=xl_file_path)
        sheetnames = None
        if hasattr(db_tmp, 'ws_names'):
            sheetnames = db_tmp.ws_names() if callable(db_tmp.ws_names) else db_tmp.ws_names
        elif hasattr(db_tmp, 'ws'):
            try:
                sheetnames = list(db_tmp.ws.keys())
            except Exception:
                sheetnames = None

        if sheetnames:
            print('Available sheets in workbook:', sheetnames)
        else:
            print('Could not determine sheet names from workbook using pylightxl.')
    except Exception as e2:
        print('Error reading workbook to list sheets:', e2)
    sys.exit(1)
...
def process_excel_VM_details_reportNot_monitoredbyZB():
    for row in db.ws(ws=sheet_name).rows:
        ci_host_name = row[5]
        ci_portfoliio = row[11]
        ci_product = row[12]
        ci_E2customer_Hub = row[13]
        ci_hub_identifier = row[14]
        CI_json_input = {
            "host_name": ci_host_name,
            "portfolio": ci_portfoliio,
            "product": ci_product,
