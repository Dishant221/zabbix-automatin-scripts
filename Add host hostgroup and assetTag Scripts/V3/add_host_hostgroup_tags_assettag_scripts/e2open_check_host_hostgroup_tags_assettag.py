...
def process_host_row(hostgroup, tag, _, asset_tag, hostname):
    ...
    if not host_id:
        ...
        if host_id:
            ...
            print(hostname)
            #Create or get the host group for capital hostname
            group_id = create_hostgroup(hostname,hostgroup)

            #Update host group (without removing existing ones) for capital hostname
            update_host_groups(host_id, [group_id], hostgroup, hostname)

            #Update inventory asset tag for capital hostname
            update_asset_tag(host_id, asset_tag, hostname)

            #Check and assign missing tags for capital hostname
